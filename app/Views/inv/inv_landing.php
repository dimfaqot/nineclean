...
<script>
    ...
    let main = (tahun, bulan) => {
        ...
        datas.data.forEach((e, i) => {
            ...
            if (role == "Root") {
                html += `<button class="btn btn-sm btn-danger delete" data-id="${e.id}" data-is_show="show" data-message="Yakin hapus?" data-tabel="pengeluaran" data-is_reload="">Delete</button>`;
    ...
    let form_input = (order, id) => {
        ...
        datas.sub_menu.forEach((e, i) => {
            html += `<option ${(order=="Add" && i==1?"selected":(e==data.jenis?"selected":""))} value="${e}">${e}</option>`;
